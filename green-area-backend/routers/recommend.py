...
def recommend_province(province_name: str, year: int = CURRENT_YEAR):
    ...
    if cached.data:
        row = cached.data[0]
        # ลอง in-process cache ก่อน — ถ้ามี ไม่ต้อง compute GEE เลย
    ...
    logger.info("⏳ Computing recommendation: %s/%d", province_name, year)
    try:
        geom = ee.Geometry(raw_geom)
        priority, _, _, _ = _compute_priority(geom, year)
        tile_url = _get_heatmap_url(priority)
...
def recommend_district(province_name: str, district_name: str,
    ...
    logger.info("⏳ Computing recommendation: %s/%s/%d", province_name, district_name, year)
    try:
        geom = ee.Geometry(raw_geom)
        priority, _, _, _ = _compute_priority(geom, year)
        tile_url = _get_heatmap_url(priority)
